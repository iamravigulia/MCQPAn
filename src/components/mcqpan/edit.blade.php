...
@php $que = DB::table('fmt_mcqpan_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalMCQPAN{{$que->id}}">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <a onclick="closeModalMCQPAN({{$message}})" class="p-2 bg-white w-8 h-8 bg-gray-600 text-white rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-40" href="javascript:void(0);">x</a>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mcqpan.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    @endif
                    @csrf
                    <div class="text-xl">Edit MCQpan Question {{$message}}</div>
                    ...
                    @php $answers = DB::table('fmt_mcqpan_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mcqpan_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
                        <div class="flex flex-wrap my-4" id="ans1">
                            <div class="w-1/3">
                                <label class="w-full text-xs" for="">Replace it</label>
                                <input type="text" value="{{$answer->id}}" name="ans{{$answer->id}}" hidden>
                                <input class="my-2 border border-gray-500 px-2 w-full rounded-lg" type="file" accept="image/*" name="answer{{$answer->id}}" placeholder="Answer" value="{{$answer->answer}}">
                            </div>
